...
async function parseResponse(res) {
  ...
  try {
    ...
    console.debug("Failed to read response body:", error);
  }
...
const Auth = {
  ...
  get token() {
    return this._token || localStorage.getItem("tessera_token");
  },
  get username() {
    return this._username || localStorage.getItem("tessera_username");
  },
  get createdAt() {
    return this._createdAt || localStorage.getItem("tessera_createdAt");
  },
  get isLoggedIn() {
    return !!this.token;
  },
...
function dashLoadActivity() {
  try {
    return (
      JSON.parse(localStorage.getItem(dashStorageKey("activity"))) || {
        projects: 0,
        exports: 0,
        saves: 0,
      }
    ...
    return { projects: 0, exports: 0, saves: 0 };
  }
}
...
function openDashboard() {
  ...
  // Username & meta
  document.getElementById("dash-username").textContent = capitalise(
    Auth.username,
  );
  ...
  if (prefs.gridRes) {
    document.getElementById("dash-pref-grid").value = prefs.gridRes;
  }
  if (prefs.layerHeight) {
    document.getElementById("dash-pref-layer").value = prefs.layerHeight;
  }
}
...
async function dashChangeUsername() {
  ...
  if (!newUsername || !password) {
    return dashSetMsg("dash-username-msg", "Fill in all fields.", false);
  }
  ...
  try {
    ...
    const errMsg =
      parsed.data?.error || parsed.text || `Failed (${parsed.status})`;
    if (!parsed.ok) {
...
async function dashChangePassword() {
  ...
  if (!curPassword || !newPassword) {
    return dashSetMsg("dash-password-msg", "Fill in all fields.", false);
  }
...
function dashRenderFilaments() {
  ...
  if (list.length === 0) {
    container.innerHTML =
      '<div class="dash-filament-empty">No filaments added yet.</div>';
    return;
  }
