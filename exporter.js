...
const Exporter = {
  ...
  async export(layersData, onProgress) {
    ...
    for (const layer of layersData) {
      ...
      for (let j = 0; j < verts.length; j += 3) {
        lines.push(
          `v ${verts[j].toFixed(4)} ${verts[j + 1].toFixed(4)} ${verts[j + 2].toFixed(4)} ${rStr} ${gStr} ${bStr}\n`,
        );
      }
      ...
      if (typeof onProgress === "function") {
        onProgress(
          (processedLayers / totalLayers) * 100,
          `Writing OBJ data ${processedLayers}/${totalLayers}...`,
        );
      }
    ...
    try {
      ...
      if (typeof onProgress === "function") {
        onProgress(100, `Error during download: ${e.message}`);
      }
    }
  },
  ...
  async exportSTL(meshes, onProgress) {
    if (typeof onProgress === "function") {
      onProgress(0, "Counting triangles...");
    }
    ...
    for (const mesh of meshes) {
      ...
      for (let t = 0; t < numTris; t++) {
        ...
        // Vertex 1 (12 bytes)
        view.setFloat32(offset, verts[i0], true);
        offset += 4;
        view.setFloat32(offset, verts[i0 + 1], true);
        offset += 4;
        view.setFloat32(offset, verts[i0 + 2], true);
        offset += 4;

        // Vertex 2 (12 bytes)
        view.setFloat32(offset, verts[i1], true);
        offset += 4;
        view.setFloat32(offset, verts[i1 + 1], true);
        offset += 4;
        view.setFloat32(offset, verts[i1 + 2], true);
        offset += 4;

        // Vertex 3 (12 bytes)
        view.setFloat32(offset, verts[i2], true);
        offset += 4;
        view.setFloat32(offset, verts[i2 + 1], true);
        offset += 4;
        view.setFloat32(offset, verts[i2 + 2], true);
        offset += 4;
      ...
      if (typeof onProgress === "function") {
        onProgress(
          (meshIndex / meshes.length) * 100,
          `Writing STL mesh ${meshIndex}/${meshes.length}...`,
        );
      }
      await this._yieldToUI();
    }
